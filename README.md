...
> **当前版本：0.1.1 私有预览版。** 已增加正文富文本、独立标题和 PNG 图片复制。核心实现已进入验收阶段，尚未完成完整浏览器回归与真实 X 账户联调。当前没有 X 登录、上传、创建草稿或发布功能。“本地结构校验通过”不表示 X 已接受内容。详细证据与限制见 [验收记录](docs/ACCEPTANCE.md)。
...
| 能力 | 当前实现 |
|---|---|
| 写作 | CodeMirror Markdown 编辑、标题、封面、撤销重做、搜索 |
...
| 运行 | 非特权 Nginx / Docker / Caddy HTTPS | 同源静态分发，无稿件服务端 API |
...
容器使用非特权 Nginx、只读根文件系统、临时 `/tmp`、资源上限和日志轮转。宿主机仅绑定 `127.0.0.1:5701`，不要把应用端口公开到互联网。
...
- 文稿、原图与历史保存在当前浏览器 IndexedDB。服务器没有接收稿件的 API。
...
- 前端与服务端部署均不包含 API Key、Cookie 或 OAuth token。
...
| V1.1 | Mermaid 图形、有限块操作 | 未实施；手动富文本/图片复制已提前在 0.1.1 加入 |
| V2.0 | OAuth、媒体上传、真实 X 草稿桥 | 未实施，依赖账户联调 |
| V2.1 | 绑定账号/文章/版本的一次性发布授权 | 未实施 |
