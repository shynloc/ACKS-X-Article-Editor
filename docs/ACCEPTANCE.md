...
生产构建 `e8316bef2fb32315` 已部署，封面和架构 WebP 公开资源返回正确 MIME，静态与发布桥容器 healthy。浏览器应用更新后，介绍稿真实进入现有文稿库并成功打开，封面与架构图可见，转换摘要显示待修复 0。公开前使用 Gitleaks 扫描全部 11 个提交和当前源码，未发现密钥；管理员邀请码、本地数据库和 `.env` 均未被 Git 跟踪。仓库已加入 MIT License，并切换为 PUBLIC，主页和十个项目 topics 已设置。
